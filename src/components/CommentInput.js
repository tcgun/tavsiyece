    ...
    <form onSubmit={(e) => onSubmit(e)} className="flex items-start space-x-3 p-4 border-t">
      ...
      <div className="flex-1">
        <textarea
          ...
          rows="2"
          className="w-full p-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition"
        />
        <button
          type="submit"
          disabled={!text.trim()}
          className="mt-2 float-right bg-teal-600 text-white font-bold py-2 px-5 rounded-lg hover:bg-teal-700 disabled:bg-gray-300 transition-colors"
        >
          Gönder
        </button>
      </div>
